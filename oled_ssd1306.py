# oled_ssd1306.py - SSD1306 128x64 I2C 显示驱动（由 oled_display 按配置加载）
import sys
...
def update_display(
    ...
    try:
        ...
        if not _state_multi.get("oled_error_logged"):
            _log.warning("oled_ssd1306 update_display: %s" % e)
...
def update_position(i2c, lat_disp, lon_disp, gnss_type, update_time, time_dif, speed_kmh, bat_pct=None):
    ...
    try:
        ...
        if not s.get("oled_error_logged"):
            _log.warning("oled_ssd1306 update_position: %s" % e)
...
def update_display_compact(
    ...
    try:
        ...
        if not sc.get("oled_error_logged"):
            _log.warning("oled_ssd1306 update_display_compact: %s" % e)
