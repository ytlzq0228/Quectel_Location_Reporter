...
2. **把以下文件拷入模组**（如通过 QPYcom 文件管理）：
   ...
   - 若使用 Traccar 远程指令（SET/GET/SCREEN/FOTA 等）：`traccar_report.py`、`cmd_osmand.py`；若需 **FOTA UPDATE** 指令：`fota_update.py`

3. 在 REPL 或开机自启中运行 **`main.py`** 或 **`GNSS_Reporter.py`** 即可。设备上配置文件路径为 **`/usr/config.cfg`**。
...
├── config.py           # 统一配置读取（Traccar/APRS/LBS）+ 原始 key 读写供远程指令使用
├── cmd_osmand.py       # Osmand 协议指令解析与执行（SET/GET/DEL/REBOOT/SCREEN OFF|ON/FOTA UPDATE）
├── main.py             # 启动入口
├── GNSS_Reporter.py    # 主程序逻辑（GNSS + LBS + Traccar + APRS + 缓存 + 刷机检测 + 额外缓存线程）
├── traccar_report.py   # Traccar 上报模块（Queue + 消费者线程 + 备份线程 + 响应指令与 lastCmdResult 上报）
...
| **Traccar 上报** | HTTP GET 上报到 Traccar（OSM 协议），设备 ID 为模组 IMEI；支持从响应 body 接收 Osmand 指令（SET/GET/DEL/REBOOT/SCREEN OFF|ON/FOTA UPDATE）并上报 lastCmdResult |
...
- **流程**：设备 GET 上报位置 → 若响应 body 非空则视为指令原文 → `cmd_osmand` 解析执行 → 立即再发一次仅带 `id` 与 `lastCmdResult` 的 GET 上报结果 → 若指令要求重启则设备重启。
- **支持指令**：
  - **SET** `key=value`（可多组空格分隔）、**GET** `key`、**GET ALL**、**DEL** `key`、**REBOOT**：配置读写与重启。`DEL` 禁止删除 `traccar_host`、`traccar_port`。
  - **SCREEN OFF** / **SCREEN ON**：远程熄屏/亮屏，状态**仅存内存**，重启后恢复默认（亮屏）。
  - **FOTA UPDATE**：触发 FOTA 升级（后台执行，完成后设备自动重启）。
- **配置与实现**：设备上配置文件路径固定为 `/usr/config.cfg`。`cmd_osmand.py` 解析与执行；`config.py` 提供 `get_raw_value`/`set_raw_key`/`del_raw_key`/`get_all_raw` 供 SET/GET/DEL 读写配置；SCREEN 状态通过 `set_screen_on_remote`/`get_screen_on_remote` 仅内存维护。
- 多行指令只执行第一行有效指令。
